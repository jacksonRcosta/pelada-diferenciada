...
export default function ScoutModal({ pid, players, teams, open, onClose, onScoutChange, onCardChange, viewOnly }) {
  ...
  function chgCard(cid, delta) {
    ...
    onCardChange(pid, cid, delta, cards)
  }
  ...
  return (
    <Modal open={open} onClose={onClose}>
      ...
      <div style={{fontSize:10,fontWeight:800,letterSpacing:1,textTransform:'uppercase',color:'var(--t3)',padding:'10px 14px 4px',background:'#f9f8f5',borderTop:'1px solid rgba(0,0,0,.06)',borderBottom:'1px solid rgba(0,0,0,.06)'}}>Scouts (contam pontos)</div>
          ...
          return (
            <div key={s.id} style={{display:'flex',alignItems:'center',gap:9,padding:'11px 0',borderBottom:'1px solid rgba(0,0,0,.06)'}}>
              <div style={{flex:1,minWidth:0}}>
                ...
                <span style={{fontSize:11,fontWeight:700,padding:'2px 8px',borderRadius:8,background:s.c.bg,color:s.c.dk,display:'inline-block',marginTop:3}}>{s.pts>0?'+':''}{s.pts} pts</span>
              </div>
      ...
      <div style={{fontSize:10,fontWeight:800,letterSpacing:1,textTransform:'uppercase',color:'var(--t3)',padding:'10px 14px 4px',background:'#f9f8f5',borderTop:'1px solid rgba(0,0,0,.06)',borderBottom:'1px solid rgba(0,0,0,.06)'}}>Cartões (só marcação)</div>
