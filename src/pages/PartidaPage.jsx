...
import { calcPoints, ptStyle, ptsLabel, formatTime, initials, avatarColor, buildSchedule } from '../lib/utils'
...
export default function PartidaPage({ state, update, viewOnly, onOpenScout }) {
  ...
  if (!teams || !teams.length) {
    return <div style={{ textAlign: 'center', padding: '40px 12px', color: 'var(--t3)', fontSize: 13, lineHeight: 1.9 }}>
  ...
  function setMatch(mi) {
    const g = schedule[mi]
    if (!g) return
    update({ activeMatch: mi, matchA: g.a, matchB: g.b,
      scoreA: g.done ? g.scoreA : 0, scoreB: g.done ? g.scoreB : 0,
  ...
  function doSub(newPid) {
    ...
    update({ teams: nt })
    setSubPid(null); setSubTidx(-1)
    showToast(`${op?.name} ⇄ ${np?.name}`)
  }

  const cands = subTidx >= 0 && teams[subTidx]
    ? players.filter(p => !teams[subTidx].pids.includes(p.id)) : []
  const tc = timer.status === 'running' ? 'var(--green)' : timer.status === 'overtime' ? 'var(--red)' : 'var(--navy)'
  const card = (style={}) => ({ background: 'var(--sur)', borderRadius: 14, border: '1px solid var(--brd)', marginBottom: 10, overflow: 'hidden', ...style })
  ...
  return (
    <div>
      ...
      {tmA && tmB && (
        <div style={{ background: 'linear-gradient(135deg,var(--navy),var(--navy2))', borderRadius: 16, marginBottom: 12, overflow: 'hidden' }}>
          ...
          <div style={{ textAlign: 'center', padding: 8, fontSize: 12, fontWeight: 700, color: matchFinished ? 'var(--gold)' : 'rgba(255,255,255,.7)', borderTop: '1px solid rgba(255,255,255,.1)' }}>
            {matchFinished ? `🏆 Encerrada — ${tmA.name} ${scoreA} × ${scoreB} ${tmB.name}` : `Jogo ${(activeMatch ?? -1) + 1} em andamento`}
          </div>
        </div>
      )}
      ...
      {tmA && tmB && !matchFinished && (
        <div style={card()}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 13px', background: '#f0ede8', borderBottom: '1px solid var(--brd)', flexWrap: 'wrap' }}>
            ...
            <input type="number" defaultValue={45} min={1} max={120} onChange={e => timer.setMin(+e.target.value || 45)}
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 13px', background: '#f9f8f5' }}>
            ...
            <div style={{ flex: 1, textAlign: 'center', fontSize: 28, fontWeight: 800, fontFamily: 'monospace', fontVariantNumeric: 'tabular-nums', color: tc }}>{formatTime(timer.secs)}</div>
            {timer.running
                ...
                {pls.map(pp => {
                  ...
                  return (
                    <button key={pp.id} onClick={() => onOpenScout(pp.id)}
                      ...
                      <div style={{ width: 30, height: 30, borderRadius: '50%', background: bg, color: fg, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 800, flexShrink: 0 }}>{initials(pp.name)}</div>
            ...
            return (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 13px', borderBottom: i < schedule.length - 1 ? '1px solid rgba(0,0,0,.05)' : 'none' }}>
                ...
                <div style={{ display: 'flex', gap: 5 }}>
                  {!viewOnly && (
                    <button onClick={() => { const ns = schedule.map((s, si) => si === i ? { ...s, a: s.b, b: s.a } : s); update({ schedule: ns }) }}
      ...
      ) : <div style={{ ...card(), padding: 16, textAlign: 'center', color: 'var(--t3)', fontSize: 13 }}>Sorteie os times para gerar a agenda.</div>}
          ...
          : cands.map(c => {
            ...
            return (
              <button key={c.id} onClick={() => doSub(c.id)}
                ...
                <div style={{ width: 36, height: 36, borderRadius: '50%', background: bg, color: fg, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 800 }}>{initials(c.name)}</div>
                <div><div style={{ fontSize: 14, fontWeight: 700 }}>{c.name}</div><div style={{ fontSize: 12, color: 'var(--t3)' }}>{c.pos}</div></div>
              </button>
            )
          })
        }
