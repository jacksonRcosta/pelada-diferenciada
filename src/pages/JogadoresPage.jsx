...
export default function JogadoresPage({ state, update, viewOnly }) {
  ...
  function add() {
    const n = name.trim()
    if (!n) return
    if (players.some(p => p.name.toLowerCase() === n.toLowerCase())) { showToast('Peladeiro já existe!'); return }
    const p = { id: state.nextId, name: n, pos, sc: {}, cards: {} }
    update({ players: [...players, p], nextId: state.nextId + 1 })
    setName('')
  ...
  function del(pid) {
    ...
    const newTeams = teams ? teams.map(t => ({ ...t, pids: t.pids.filter(id => id !== pid) })) : teams
  ...
  const inputStyle = { width: '100%', padding: '12px 13px', fontSize: 16, border: '1.5px solid #e0ddd6', borderRadius: 10, background: '#f9f8f5', color: 'var(--txt)', marginBottom: 10 }
  const labelStyle = { display: 'block', fontSize: 10, fontWeight: 800, letterSpacing: '.8px', textTransform: 'uppercase', color: 'var(--t3)', marginBottom: 5 }

  return (
    <div>
      {!viewOnly && (
        <>
          <div style={{ fontSize: 10, fontWeight: 800, letterSpacing: 1, textTransform: 'uppercase', color: 'var(--t3)', marginBottom: 7 }}>Novo Peladeiro</div>
          <div style={{ background: 'var(--sur)', borderRadius: 14, border: '1px solid var(--brd)', padding: 14, marginBottom: 10 }}>
            <label style={labelStyle}>Nome</label>
            <input value={name} onChange={e => setName(e.target.value)} onKeyDown={e => e.key === 'Enter' && add()}
              placeholder="Ex: Juninho" maxLength={24} style={inputStyle} />
            <label style={labelStyle}>Posição</label>
            <select value={pos} onChange={e => setPos(e.target.value)} style={{ ...inputStyle, marginBottom: 0 }}>
              {POSITIONS.map(p => <option key={p}>{p}</option>)}
            </select>
            <button onClick={add} style={{ width: '100%', padding: 14, borderRadius: 11, background: 'var(--navy)', color: '#fff', fontSize: 15, fontWeight: 700, marginTop: 10 }}>
              + Adicionar Peladeiro
            </button>
          </div>
        </>
      ...
      {players.length === 0 && <div style={{ textAlign: 'center', padding: '40px 12px', color: 'var(--t3)', fontSize: 13 }}>Nenhum peladeiro ainda.</div>}
        ...
        return (
          <div key={p.id} style={{ background: 'var(--sur)', borderRadius: 14, border: '1px solid var(--brd)', marginBottom: 10, overflow: 'hidden' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 13px' }}>
              <div style={{ width: 40, height: 40, borderRadius: '50%', background: bg, color: fg, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 800 }}>{initials(p.name)}</div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 15, fontWeight: 700, display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: 4 }}>
                  {p.name}
                  {tc && <span style={{ background: tc.color, color: '#fff', fontSize: 10, fontWeight: 700, padding: '1px 7px', borderRadius: 6 }}>{teams[ti].name}</span>}
                </div>
                <div style={{ fontSize: 12, color: 'var(--t3)', marginTop: 2 }}>{p.pos}</div>
              </div>
              {!viewOnly && (
                <>
                  <button onClick={() => { setEditPid(p.id); setEditPos(p.pos) }}
      ...
      <Modal open={!!editPid} onClose={() => setEditPid(null)}>
        <div style={{ padding: '13px 16px 12px', borderBottom: '1px solid rgba(0,0,0,.07)', position: 'relative' }}>
          <div style={{ fontSize: 17, fontWeight: 800 }}>{players.find(p => p.id === editPid)?.name}</div>
